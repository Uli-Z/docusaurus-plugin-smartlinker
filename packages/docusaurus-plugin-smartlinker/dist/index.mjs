import { dirname, resolve, join, isAbsolute, normalize, relative } from 'path';
import { readdirSync, statSync, readFileSync, utimesSync, existsSync } from 'fs';
...
function resolveStaticDirs(siteDir, relative7) {
  return (relative7 ?? []).map((dir) => join(siteDir, dir));
}
...
var startTimer = (log, ...levels) => shouldMeasure(log, ...levels) ? performance.now() : null;
var endTimer = (start) => {
