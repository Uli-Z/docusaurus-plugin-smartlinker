import { dirname, resolve, relative, isAbsolute, join, normalize } from 'path';
import { existsSync, utimesSync, readdirSync, statSync, readFileSync } from 'fs';
...
function smartlinkerPlugin(_context, optsIn) {
  ...
  const toAbsolutePath = (filePath) => isAbsolute(filePath) ? filePath : resolve(_context.siteDir, filePath);
