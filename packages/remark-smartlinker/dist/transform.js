...
import { getIndexProvider as getRegisteredIndexProvider, PLUGIN_NAME, resolveDebugConfig, createLogger, getDebugConfig, } from 'docusaurus-plugin-smartlinker';
...
export default function remarkSmartlinker(opts) {
    ...
    if (debugResolution.invalidLevel && typeof console !== 'undefined' && typeof console.warn === 'function') {
        console.warn(`[${PLUGIN_NAME}] Ignoring DOCUSAURUS_PLUGIN_DEBUG_LEVEL="${debugResolution.invalidLevel}" (expected one of: error, warn, info, debug, trace).`);
    }
    ...
    return (tree, file) => {
        ...
        const onLinkInserted = (args) => {
            if (!transformLogger.isLevelEnabled('debug'))
                return;
...
function transformText(args) {
    const { text, matcher, claimMap, componentName, toAttr, tipKeyAttr, matchAttr, iconAttr, shortNoteComponentName, shortNoteTipKeyAttr, shortNotePlaceholder, currentTarget, onLinkInserted, onShortNoteInserted, } = args;
    ...
    if (hasPlaceholder && currentTarget) {
        ...
        for (let i = 0; i < segments.length; i++) {
            ...
            if (i < segments.length - 1) {
                ...
                if (onShortNoteInserted)
                    onShortNoteInserted({ id: currentTarget.id });
                changed = true;
            }
        }
...
function transformSegment(args) {
    ...
    for (const m of matches) {
        ...
        if (currentTarget && id && currentTarget.id === id) {
            ...
            if (onLinkInserted)
                onLinkInserted({ text: m.text, slug, id, icon });
        }
        cursor = end;
    }
