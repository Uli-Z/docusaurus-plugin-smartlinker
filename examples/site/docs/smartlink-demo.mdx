...
sidebar_position: 4
description: Draft notes on how the Smartlinker plugin behaves inside the antibiotic guideline example.
---
...
The remark plugin runs on every docs and pages pipeline, so smartlinking applies to more than plain paragraphs. A few examples that rely on the same index:
